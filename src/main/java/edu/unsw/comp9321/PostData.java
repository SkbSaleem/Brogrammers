...
public class PostData {
	...
	public void updatePost(int postId, String content) {
		...
		try {
			tx = session.beginTransaction();
			// Query query = session.createSQLQuery("delete from posts where PostId =
			// :postid").setParameter("postid", postId);
	...
	public void likePost(int postId) {
		...
		try {
			tx = session.beginTransaction();
			// Query query = session.createSQLQuery("update posts p set p.Likes = p.Likes +
			// 1" + "where p.PostId = postid").setParameter("postid", postId);
			PostPojo p = (PostPojo) session.get(PostPojo.class, postId);
